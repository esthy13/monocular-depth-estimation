...
class DepthAnyCamera(DepthModel):
    ...
    Args:
        ...
        fwd_sz:     optional model input size as ``(height, width)``.
        device:     'cpu', 'cuda', or 'mps'. Auto-detected if None.
        config_path: optional local DAC JSON config, used by the Colab cache.
        checkpoint_path: optional local DAC checkpoint, used by the Colab cache.
    """
    ...
    def set_camera(self, cam_params: dict, crop_wfov: float) -> None:
        """Switch calibrated cameras without reloading the model weights."""
    ...
    def _erp_crop_shape(
        ...
        """Return an ERP crop ``(height, width)`` matched to model input."""
    ...
    def load(self) -> None:
        ...
        if self._config_path is None:
            print(f"Downloading/caching DAC config: {config_file}")
            ...
            config_path = Path(self._config_path).expanduser().resolve()
        if self._checkpoint_path is None:
            print(f"Downloading/caching DAC weights: {weights_file} (~700 MB)")
            ...
            weights_path = Path(self._checkpoint_path).expanduser().resolve()
        if not config_path.is_file():
            raise FileNotFoundError(f"DAC config not found: {config_path}")
        if not weights_path.is_file():
            raise FileNotFoundError(f"DAC checkpoint not found: {weights_path}")
    ...
    def predict(self, image: np.ndarray) -> np.ndarray:
        ...
        if is_fisheye:
            ...
            mask_in = lens_mask.astype(np.float32)[..., None]
        else:
        ...
        crop_h, crop_w = self._erp_crop_shape(
            cano_sz[0], fwd_sz, self._crop_wfov, is_fisheye
        )
        self.last_projection_metadata = {
            "requested_horizontal_fov_degrees": float(self._crop_wfov),
            "erp_crop_height_pixels": int(crop_h),
            "erp_crop_width_pixels": int(crop_w),
            "erp_vertical_fov_degrees": float(180.0 * crop_h / cano_sz[0]),
            "erp_horizontal_fov_degrees": float(180.0 * crop_w / cano_sz[0]),
            "model_input_height": int(fwd_sz[0]),
            "model_input_width": int(fwd_sz[1]),
            "full_circular_fisheye_coverage": bool(is_fisheye),
            "input_valid_fraction": float(mask_in.mean()),
        }
        ...
        with torch.no_grad():
            if model_name == "IDiscERP":
                preds, _, _ = self._model(
                    ...
                    attn_mask=attn_tensor.to(self.device),
                )
            else:
        ...
        _img_out, depth_out, valid_out, active_out = erp_patch_to_cam_fast(
        ...
        valid = valid_out[0, 0].numpy() >= 0.5
        active = active_out[0].numpy() >= 0.5
        depth_cam[~(valid & active & np.isfinite(depth_cam) & (depth_cam > 0))] = 0.0
        return depth_cam
