...
both physical references and a positive distance bias. This result alone does
not show whether UniDAC is better than DAC. The next required experiment is to
run DAC on the identical 133 frames (or the identical 14-frame sample first)
and compare summaries with `compare_evaluation_runs.py`.
