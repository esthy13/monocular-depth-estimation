...
| Depth Any Camera (DAC) | Yes | Uses the camera model, focal lengths, principal point, and fisheye distortion to project the image to an equirectangular representation and back; the matched Colab baseline widens the ERP crop to retain the full circular G1_A field | Metric Euclidean ray distance in metres |
...
DAC and UniDAC outputs are metric depth, where smaller values are closer. Their
matched-run metadata records `metric Euclidean ray distance`; the 3D point at
image pixel `(u, v)` is therefore:
